...
export default function PathwayVisualization({ pathwayData, regulatoryData, geneMapping = {} }: PathwayVisualizationProps) {
    ...
    useEffect(() => {
        ...
        // Group node content by node_id for quick lookup
        const nodeContentById = new Map<string, string[]>();
        pathwayData.nodeContent.forEach(nc => {
            const list = nodeContentById.get(nc.node_id);
            if (list) {
                list.push(nc.gene_or_compound_id);
            } else {
                nodeContentById.set(nc.node_id, [nc.gene_or_compound_id]);
            }
        });
    ...
    }, [pathwayData, regulatoryData, showLabels, highlightRegulated, geneMapping]);
